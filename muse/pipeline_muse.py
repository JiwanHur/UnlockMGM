...
class PipelineMuse:
    def __init__(
        ...
        transformer: Union[MaskGitTransformer],
    ...
    def __call__(
        ...
        use_tqdm=True,
        use_maskgit_c_generate: bool = False,
        max_c_timestep:int = 1000,
        ...
        **kwargs,
        ...
        if use_maskgit_c_generate:
            generate = self.transformer.generate4
        ...
        if return_intermediate:
            ...
            # mask_index = [[self.to_pil_image(image.reshape(-1,16,16)[0]) for image in images] for images in mask_index]
    ...
    def from_pretrained(
        ...
        if model_name_or_path is None:
            if vae_path is None or transformer_path is None:
                raise ValueError(
                    "If model_name_or_path is None, then vae_path, and transformer_path must be"
                    " provided."
                )
        ...
        transformer_config = MaskGitTransformerOrg.load_config(**transformer_args)
        if transformer_config["_class_name"] == "MaskGitTransformerOrg":
            transformer = MaskGitTransformerOrg.from_pretrained(**transformer_args)
