...
def gumbel_sample_max(t, temperature=1.0, dim=-1, generator=None):
    return ((t / max(temperature, 1e-10)) + gumbel_noise(t, generator=generator)).max(dim=dim)
...
def exponential_schedule(t, total_unkown=256):
    mask_ratio = 1 - torch.exp(-torch.log(torch.tensor(total_unkown)) * (1-t))
    return mask_ratio

def sqrt_schedule(t):
    return (1 - torch.sqrt(t)).clamp(min=1e-6, max=1.0)
