...
def convert(flax_model_path, pytorch_dump_folder_path, num_vq_tokens):
    ...
    pt_model = MaskGitTransformer(
        ...
        max_position_embeddings=num_vq_tokens+1, # num_vq_tokens + 1 for class token
        hidden_size=768,
...
if __name__ == "__main__":
    ...
    flax_model_path = f'./scripts/maskgit_imagenet{resolution}_checkpoint'
    pytorch_dump_folder_path = f'./scripts/maskgit_imagenet{resolution}_torch'
    num_vq_tokens = 256 if resolution == 256 else 1024
    convert(flax_model_path, pytorch_dump_folder_path, num_vq_tokens)