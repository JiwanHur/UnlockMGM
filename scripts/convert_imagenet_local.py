...
def convert_imagenet_to_wds(output_dir, max_train_samples_per_shard, max_val_samples_per_shard):
    ...
    # dataset = load_dataset("imagenet-1k", split="train", data_dir='/mnt/sdb/datset/ILSVRC2012')
    dataset = load_dataset("imagenet-1k", split="train", data_dir='/mnt/sdb/ILSVRC2012/')
    print('load data')
    now = time.time()
    ...
    dataset = load_dataset("imagenet-1k", data_dir='/mnt/sdb/ILSVRC2012/', split="validation")
    now = time.time()
    for i, example in enumerate(dataset):
