    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(model):
                ...
                # forward for correction loss
                # try:
                #     if config.training.use_correction_loss:
                #         if config.training.correction_loss_version == "v1":
                #             gen_corr_loss = model.gen_corr_loss
                #         elif config.training.correction_loss_version == "v2":
                #             gen_corr_loss = model.gen_corr_loss_2
                #         elif config.training.correction_loss_version == "v3":
                #             gen_corr_loss = model.gen_corr_loss_3
                #         elif config.training.correction_loss_version == "v4":
                #             gen_corr_loss = model.gen_corr_loss_4
                #         elif config.training.correction_loss_version == "v5":
                #             gen_corr_loss = model.gen_corr_loss_5
                #         else:
                #             raise NotImplementedError("Please set config.training.correction_loss_version")
                            
                #         #TODO: Add soft code target 
                #         _, correction_loss = gen_corr_loss(
                #             input_ids=input_ids, labels=labels, label_smoothing=config.training.label_smoothing,
                #             substitution_rate=config.training.substitution_rate
                #         )
                #         avg_corr_loss = accelerator.gather(correction_loss.repeat(config.training.batch_size)).mean()
                #         accelerator.backward(correction_loss)
                #     else:
                #         avg_corr_loss = torch.tensor(0.0, device=accelerator.device)
                # except:
                #     raise ValueError("Please set config.training.use_correction_loss")
                #TODO: Add soft code target 
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step + 1) % config.experiment.log_every == 0:
                    ...
                    logger.info(
                        ...
                        # f"Loss_Corr: {avg_corr_loss.item():0.4f} "
...
def generate_images(model, vq_model, accelerator, global_step):
    ...
    with torch.autocast("cuda", dtype=dtype, enabled=accelerator.mixed_precision != "no"):
        gen_token_ids = accelerator.unwrap_model(model).generate2(class_ids=imagenet_class_ids, timesteps=8)
